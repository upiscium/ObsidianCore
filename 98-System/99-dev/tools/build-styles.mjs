...
export const ACTIVATION_CONVERGENCE_MARKER = "98-System/99-dev/design/appearance-promotion-convergence.json";
...
export function readActivationConvergence(root) {
  const markerPath = path.join(root, ACTIVATION_CONVERGENCE_MARKER);
  if (!fs.existsSync(markerPath)) return null;

  const marker = JSON.parse(fs.readFileSync(regularPath(root, ACTIVATION_CONVERGENCE_MARKER), "utf8"));
  if (marker?.schema_version !== 1 || marker?.mode !== "appearance-live-promotion-convergence") {
    throw new Error("unsupported appearance promotion convergence marker");
  }
  if (!Array.isArray(marker.observed_managed_snippets)
      || JSON.stringify(marker.observed_managed_snippets) !== JSON.stringify(LEGACY_SNIPPETS)) {
    throw new Error("appearance convergence marker must bind the exact legacy activation order");
  }
  if (JSON.stringify(marker.target_managed_snippets) !== JSON.stringify(["obsidian-core", "obsidian-core-mobile"])) {
    throw new Error("appearance convergence marker must target the canonical Core activation");
  }
  if (typeof marker.observed_theme !== "string" || typeof marker.observed_css_theme !== "string") {
    throw new Error("appearance convergence marker must bind theme and cssTheme");
  }
  return marker;
}
...
export function checkActivation(root) {
  ...
  const appearance = JSON.parse(source);
  const names = appearance.enabledCssSnippets;
  const convergence = readActivationConvergence(root);

  if (convergence) {
    if (appearance.theme !== convergence.observed_theme
        || appearance.cssTheme !== convergence.observed_css_theme
        || JSON.stringify(names) !== JSON.stringify(convergence.observed_managed_snippets)) {
      throw new Error("appearance does not match the explicitly recorded Live convergence state");
    }
    return;
  }
