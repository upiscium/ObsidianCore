...
All section controls keep `style: primary` as the fallback appearance, with their
existing labels and icons. The subsequent shared style bundle adds only the
opt-in `class: oc-action`; see `tokyo-night-style-bundle.md` for CSS rollout.
...
intentional `default` to `primary` style change and exact `class: oc-action` line.
