...
function makeFakeVault(entries) {
  ...
  const systemFiles = new Map([[referencePath, makeFile(referencePath)], [noteMetaPath, makeFile(noteMetaPath)]]);
  const notices = [];
  const mutations = [];
  const app = {
    vault: {
      ...
      read: async file => file.path === referencePath ? referenceSource : file.path === noteMetaPath ? noteMetaSource : ""
    },
    ...
    fileManager: {
      processFrontMatter: async (file, mutator) => {
        ...
        const before = structuredClone(fm); mutator(fm); mutations.push({ path: file.path, before, after: structuredClone(fm) });
      },
      generateMarkdownLink: target => `[[${target.path.replace(/\.md$/, "")}|${target.basename}]]`
    }
  };
  return { app, notices, mutations, getFrontmatter: filePath => frontmatter.get(filePath), Notice: function Notice(message) { notices.push(String(message)); } };
}

function loadMigration(env) {
  const module = { exports: {} }; const quietConsole = { log() {}, warn() {}, table() {} };
  const crypto = { randomUUID: () => "00000000-0000-4000-8000-000000000000" };
  new Function("module", "app", "Notice", "crypto", "console", migrationSource)(module, env.app, env.Notice, crypto, quietConsole);
  return module.exports;
}
function loadDoctor(env) {
  const module = { exports: {} }; const quietConsole = { log() {}, warn() {}, table() {} }; const window = { moment: makeMoment() };
  new Function("module", "app", "Notice", "window", "console", doctorSource)(module, env.app, env.Notice, window, quietConsole);
  return module.exports;
}
async function migrateAndDoctor(entries) {
  const env = makeFakeVault(entries); const migrate = loadMigration(env); const doctor = loadDoctor(env);
  const report = await migrate({}); const diagnosis = await doctor({}); return { env, report, diagnosis };
}

function workspace(folder, name = folder, extra = {}) {
  return {
    path: `03-Workspace/${folder}/${name}.md`,
    fm: { type: "workspace", uid: `ws_${name.toLowerCase()}`, title: name, aliases: [], lifecycle: "active", ...extra }
  };
}
function project(folder, name = folder, workspaceLink, extra = {}) {
  return {
    path: `10-Project/${folder}/${name}.md`,
    fm: { type: "project", uid: `prj_${name.toLowerCase()}`, title: name, aliases: [], status: "running", priority: "high", workspace: workspaceLink, ...extra }
  };
}
function canonicalNote(type, extra = {}) { return { type, lifecycle: "active", category: null, aliases: [], tags: [], ...extra }; }
function task(fileName, extra = {}) {
  return {
    path: `02-Task/2026/08/${fileName}.md`,
    fm: { type: "task", title: fileName, status: "todo", priority: "medium", completed: null, start: null, due: null, workspace: null, project: null, triaged: false, backlog: false, depends_on: [], ...extra }
  };
}
...
test("real migration infers a missing Workspace Note relation and Doctor becomes clean", async () => {
  ...
  const { env, report, diagnosis } = await migrateAndDoctor([
    ...
    { path: notePath, fm: canonicalNote("workspace-note", { title: "Meeting", marker: "keep" }) }
  ]);
  assert.equal(env.getFrontmatter(notePath).workspace, researchLink);
  assert.equal(env.getFrontmatter(notePath).marker, "keep");
  assert.equal(report.inferred, 1); assert.equal(report.ambiguous.length, 0); assert.equal(report.unresolved.length, 0);
...
test("real migration infers Project and derives Workspace for a Project Note", async () => {
  ...
  const { env, report, diagnosis } = await migrateAndDoctor([
    workspace("Research", "Research"), project("Terreate", "Terreate", researchLink),
    { path: notePath, fm: canonicalNote("project-note", { title: "Design", marker: "keep" }) }
  ]);
  const fm = env.getFrontmatter(notePath);
  assert.equal(fm.project, terreateLink); assert.equal(fm.workspace, researchLink); assert.equal(fm.marker, "keep");
  assert.equal(report.inferred, 2); assert.equal(report.ambiguous.length, 0); assert.equal(report.unresolved.length, 0);
...
test("ambiguous parent Entries are reported without mutating the relation", async () => {
  ...
  const env = makeFakeVault([
    workspace("Shared", "Alpha", { uid: "ws_alpha" }), workspace("Shared", "Beta", { uid: "ws_beta" }),
    { path: notePath, fm: canonicalNote("workspace-note", { title: "Note", marker: "keep" }) }
  ]);
  const report = await loadMigration(env)({}); const fm = env.getFrontmatter(notePath);
  assert.equal(fm.workspace, undefined); assert.equal(fm.marker, "keep"); assert.equal(report.unresolved.length, 0); assert.equal(report.ambiguous.length, 1);
  assert.deepEqual(report.ambiguous[0], { owner: notePath, field: "workspace", value: "(parent folder)", candidates: ["03-Workspace/Shared/Alpha.md", "03-Workspace/Shared/Beta.md"] });
});

test("unresolved parent relation is reported without mutation", async () => {
  const notePath = "03-Workspace/Orphan/Note.md";
  const env = makeFakeVault([workspace("Research", "Research"), { path: notePath, fm: canonicalNote("workspace-note", { title: "Note", marker: "keep" }) }]);
  const report = await loadMigration(env)({}); const fm = env.getFrontmatter(notePath);
  assert.equal(fm.workspace, undefined); assert.equal(fm.marker, "keep"); assert.equal(report.ambiguous.length, 0); assert.equal(report.unresolved.length, 1);
...
test("real migration canonicalizes resolvable legacy Task relations and preserves unrelated metadata", async () => {
  ...
  const { env, report, diagnosis } = await migrateAndDoctor([
    workspace("Research", "Research", { aliases: ["Lab"] }), project("Terreate", "Terreate", researchLink, { aliases: ["Engine"] }),
    task("LegacyRelations", { workspace: "Lab", project: "Engine", marker: { nested: true } })
  ]);
  const fm = env.getFrontmatter(taskPath);
  assert.equal(fm.workspace, researchLink); assert.equal(fm.project, terreateLink); assert.deepEqual(fm.marker, { nested: true });
  assert.equal(report.ambiguous.length, 0); assert.equal(report.unresolved.length, 0);
