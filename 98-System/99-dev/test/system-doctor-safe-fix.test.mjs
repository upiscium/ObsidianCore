...
function record(filePath, fm, parentPath = path.posix.dirname(filePath)) {
  return {
    file: { path: filePath, basename: basename(filePath), extension: "md", parent: { path: parentPath } },
    parentPath,
    fm
  };
}
...
function canonicalRecords() {
  const workspace = record("03-Workspace/A/A.md", {
    type: "workspace", uid: "ws-a", title: "A", aliases: ["Alpha"], lifecycle: "active"
  });
  const project = record("10-Project/P/P.md", {
    type: "project", uid: "prj-p", title: "P", aliases: [], status: "running", priority: "high", workspace: "[[03-Workspace/A/A|A]]"
  });
...
test("safe planner proposes only unique deterministic relation repairs", () => {
  ...
  const projectTask = record("02-Task/2026/08/ProjectTask.md", { type: "task", project: "[[10-Project/P/P|P]]", workspace: null });
  const legacyTask = record("02-Task/2026/08/LegacyWorkspace.md", { type: "task", project: null, workspace: "Alpha" });

  const fixes = F.planSafeFixes({
    records: [workspace, project, workspaceNote, projectNote, projectTask, legacyTask],
    doctorIssues: [
      issue(workspaceNote.file.path, "workspace"), issue(projectNote.file.path, "project"),
      issue(projectTask.file.path, "workspace", "warning"), issue(legacyTask.file.path, "workspace", "warning")
    ],
    makeLink
  });

  assert.equal(fixes.length, 5);
  assert.deepEqual(fixes.map(fix => [fix.path, fix.field, fix.kind]), [
...
test("ambiguous aliases and parent-folder inference remain report-only", () => {
  const a = record("03-Workspace/Shared/A.md", { type: "workspace", uid: "ws-a", title: "A", aliases: ["Shared"], lifecycle: "active" }, "03-Workspace/Shared");
  const b = record("03-Workspace/Shared/B.md", { type: "workspace", uid: "ws-b", title: "B", aliases: ["Shared"], lifecycle: "active" }, "03-Workspace/Shared");
  ...
  const fixes = F.planSafeFixes({ records: [a, b, note, task], doctorIssues: [issue(note.file.path, "workspace"), issue(task.file.path, "workspace", "warning")], makeLink });
...
test("Doctor findings outside the safe relation scope are never mutated", () => {
  ...
  const task = record("02-Task/2026/08/Unsafe.md", {
    type: "task", status: "done", completed: null, start: "2026-08-20", due: "2026-08-10",
    workspace: "[[03-Workspace/A/A|A]]", project: "[[10-Project/P/P|P]]"
  });
  const fixes = F.planSafeFixes({ records: [workspace, project, task], doctorIssues: [issue(task.file.path, "completed"), issue(task.file.path, "start/due"), issue(task.file.path, "workspace/project")], makeLink });
  assert.deepEqual(fixes, []);
});
...
test("Safe Fix application uses an optimistic before-value precondition", () => {
  const fix = { field: "workspace", before: null, after: "[[03-Workspace/A/A|A]]" };
...
function makeFakeApp(entries) {
  const files = new Map(); const frontmatter = new Map();
  for (const entry of entries) {
    const file = record(entry.path, entry.fm).file; files.set(entry.path, file); frontmatter.set(entry.path, entry.fm);
  }
  function systemFile(filePath) {
    const absolute = path.join(root, filePath); if (!fs.existsSync(absolute)) return null;
    return { path: filePath, basename: basename(filePath), extension: path.extname(filePath).slice(1), parent: { path: path.posix.dirname(filePath) } };
  }
  return {
    app: {
      vault: {
...
test("real Safe Fix flow repairs a fake Vault only after preview and Apply", async () => {
  ...
  const entries = [
    { path: "03-Workspace/A/A.md", fm: { type: "workspace", uid: "ws-a", title: "A", aliases: [], lifecycle: "active" } },
    { path: "03-Workspace/A/Note.md", fm: { type: "workspace-note", lifecycle: "active", category: null, aliases: [], tags: [] } }
  ];
  const fake = makeFakeApp(entries);
  const notices = [];
  const previous = { app: globalThis.app, window: globalThis.window, Notice: globalThis.Notice };
  globalThis.app = fake.app; globalThis.window = { moment: makeMoment() };
  globalThis.Notice = class Notice { constructor(message) { notices.push(String(message)); } };

  let suggesterCalls = 0;
  const tp = { system: { suggester: async (_labels, values) => { suggesterCalls += 1; return values[0]; } } };
  try {
    ...
    globalThis.app = previous.app; globalThis.window = previous.window; globalThis.Notice = previous.Notice;
  }
});
