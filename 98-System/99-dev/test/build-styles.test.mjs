...
import { SOURCES, OUTPUT, LEGACY_SNIPPETS, buildCss, checkActivation, checkBundle, writeBundle, readSource, renderPreview, main } from "../tools/build-styles.mjs";
...
test("explicit input order covers all eight legacy snippets and three design modules", () => {
  assert.equal(LEGACY_SNIPPETS.length, 8); assert.equal(SOURCES.length, 11);
  assert.equal(new Set(SOURCES).size, 11);
  assert.ok(SOURCES.at(-1).endsWith("adapters.css")); assert.ok(!SOURCES.includes(OUTPUT));
});
...
test("check does not create a missing bundle", t => {
  const root = fixture(t); assert.equal(checkBundle(root), false); assert.equal(fs.existsSync(path.join(root, OUTPUT)), false);
});
test("write creates reproducible output without modifying inputs", t => {
  const root = fixture(t); const before = SOURCES.map(s => fs.readFileSync(path.join(root, s)));
  writeBundle(root); assert.equal(checkBundle(root), true); const first = fs.readFileSync(path.join(root, OUTPUT));
  writeBundle(root); assert.deepEqual(fs.readFileSync(path.join(root, OUTPUT)), first);
  SOURCES.forEach((s, i) => assert.deepEqual(fs.readFileSync(path.join(root, s)), before[i]));
});
test("stale generated bytes and changed sources fail the read-only check", t => {
  const root = fixture(t); writeBundle(root); const target = path.join(root, OUTPUT);
  fs.appendFileSync(target, "\n/* stale */"); const bytes = fs.readFileSync(target); assert.equal(checkBundle(root), false);
  assert.deepEqual(fs.readFileSync(target), bytes); writeBundle(root);
  fs.appendFileSync(path.join(root, SOURCES[0]), "\n.a {}"); assert.equal(checkBundle(root), false);
});
...
test("symlink source and output are not followed", t => {
  ...
  fs.unlinkSync(file); fs.renameSync(real, file); fs.symlinkSync(file, path.join(root, OUTPUT));
  assert.throws(() => writeBundle(root));
});
...
test("CLI check has a nonzero stale result and never fixes it automatically", t => {
  const root = fixture(t); assert.equal(main(["--check"], root), 1); assert.equal(fs.existsSync(path.join(root, OUTPUT)), false);
