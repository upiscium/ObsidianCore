...
const entityViewFactory = await loadExpression("98-System/05-lib/projects/entity_view_utils.js");
const M = entityViewFactory({ U: E, R: G, S: V });
...
if (entityType === "workspace") {
  const linkedProjects = E.isWorkspaceActiveLifecycle(current.lifecycle)
    ? allProjects.filter(project => M.projectMatchesWorkspace(project, current))
    : [];
